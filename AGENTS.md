...
- Test all: `python3 -m pytest tests/` (~593 tests, ~15s) after dev install, or `bash scripts/test-safe.sh`, or `python3 run_tests.py` (bootstraps `./env` + dev deps if pytest is missing)
